...
Usage:
    python scripts/pdf_parser.py input.pdf --out output_dir
    python scripts/pdf_parser.py input.pdf --json
"""
...
# Optional: high-resolution raster-based figure extraction
try:
...
_IMAGE_ENGINE = os.environ.get("PAPER_IMAGE_ENGINE", "pymupdf")
...
# --------------------------------------------------------------------------- #
# Pillow-based high-resolution figure extraction                               #
# --------------------------------------------------------------------------- #

def _content_bboxes_from_image(
    img: "Image.Image", min_area: int = 15000, border_margin: int = 20
) -> list[tuple[int, int, int, int]]:
    """Detect non-white content regions in a page image via thresholding.

    Returns a list of (x0, y0, x1, y1) bounding boxes in pixel coordinates.
    """
    gray = img.convert("L")
    # Invert: content → white, background → black
    inverted = ImageOps.invert(gray)
    # Threshold to binary
    threshold = 240
    binary = inverted.point(lambda p: 255 if p > 255 - threshold else 0)
    # Light blur to merge nearby elements
    blurred = binary.filter(ImageFilter.GaussianBlur(radius=4))
    # Re-threshold after blur
    binary2 = blurred.point(lambda p: 255 if p > 30 else 0)

    # Find bounding boxes of connected white regions (simple scanline approach)
    width, height = binary2.size
    pixels = binary2.load()
    ...
    for y in range(0, height, 8):
        for x in range(0, width, 8):
            ...
            if area >= min_area:
                # Add margin
                left = max(0, min_x - border_margin)
                top = max(0, min_y - border_margin)
                right = min(width, max_x + border_margin)
                bottom = min(height, max_y + border_margin)
...
def _merge_overlapping_bboxes(
    bboxes: list[tuple[int, int, int, int]], gap: int = 30
) -> list[tuple[int, int, int, int]]:
    """Merge bounding boxes that overlap or are close together (iterative)."""
    if not bboxes:
        return []
    rects = [list(b) for b in bboxes]
    changed = True
    while changed:
        changed = False
        ...
        for i, r1 in enumerate(rects):
            ...
            for j, r2 in enumerate(rects):
                if i >= j or j in used:
                    continue
                # Expand r1 by gap for overlap check
                if (
                    x0 - gap < r2[2] and x1 + gap > r2[0]
                    and y0 - gap < r2[3] and y1 + gap > r2[1]
                ):
                    x0 = min(x0, r2[0])
                    y0 = min(y0, r2[1])
                    x1 = max(x1, r2[2])
...
def extract_images_pillow(
    pdf_path: str | Path,
    output_dir: str | Path,
    page_texts: list[str] | None = None,
    dpi: int = 300,
) -> list[ExtractedImage]:
    """Extract figure regions using pdf2image + Pillow rendering.

    Renders each page as a high-resolution raster image, detects content
    regions via thresholding, and crops figures. Yields cleaner results
    for vector-heavy papers (charts, diagrams, architecture figures)
    compared to the PyMuPDF block-clustering approach.

    Requires: ``pdf2image`` + ``poppler`` (system package) + ``Pillow``.
    """
    ...
    # Also get text per page from PyMuPDF for caption detection
    doc = fitz.open(str(pdf_path))
    page_count = len(doc)
    try:
        extracted: list[ExtractedImage] = []
        for page_index in range(min(len(images), page_count)):
            pil_img = images[page_index]
            page = doc[page_index]
            ...
            scale = w / pdf_w if pdf_w > 0 else 1.0

            # Detect content regions
            bboxes = _content_bboxes_from_image(pil_img, min_area=12000)
            bboxes = _merge_overlapping_bboxes(bboxes, gap=30)

            # Filter out full-page-width regions (likely text columns, not figures)
            figure_bboxes = []
            for (x0, y0, x1, y1) in bboxes:
                region_w = x1 - x0
                region_h = y1 - y0
                aspect = region_w / max(region_h, 1)
                # Skip full-width strips (headers/footers/text columns)
                if region_w > w * 0.85 and region_h < h * 0.3:
                    continue
                # Skip tiny fragments
                if region_w < 100 or region_h < 100:
                    continue
            ...
            figure_num = 0
            for (x0, y0, x1, y1) in figure_bboxes:
                # Convert to PDF points for caption lookup
                pdf_x0, pdf_y0 = x0 / scale, y0 / scale
                pdf_x1, pdf_y1 = x1 / scale, y1 / scale
                pdf_bbox = fitz.Rect(pdf_x0, pdf_y0, pdf_x1, pdf_y1)
                ...
                figure_num += 1
                base_name = _make_figure_name(
                    page_index + 1, figure_num, block_label, block_caption
                )
                ...
                # Crop from PIL image at full resolution
                crop = pil_img.crop((x0, y0, x1, bottom))
...
def extract_images(
    ...
    """Extract figure regions (supports two backends).

    Set ``PAPER_IMAGE_ENGINE=pillow`` in environment to use the
    pdf2image + Pillow high-resolution raster pipeline.
    Default: ``pymupdf`` (block-clustering based on PyMuPDF internals).
    """
    if _IMAGE_ENGINE == "pillow" and HAS_PDF2IMAGE and HAS_PILLOW:
        return extract_images_pillow(pdf_path, output_dir, page_texts)

    return _extract_images_pymupdf(pdf_path, output_dir, page_texts)
